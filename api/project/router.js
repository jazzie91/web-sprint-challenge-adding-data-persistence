...
router.get('/resources', async (req, res) => {
  try {
    ...
    res.status(200).json({ data: formattedProjects, messages: 'Projects retrieved successfully' });
  } catch (err) {
    console.error(err);
...
router.post('/', async (req, res) => {
  ...
  if (project_description && project_description.length > 255) {
    return res.status(400).json({ message: 'Project description must be 255 characters or less' });
  }
